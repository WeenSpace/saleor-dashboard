...
test("TC: WEENSPACE_199 Create customer @e2e @customer", async () => {
...
test("TC: WEENSPACE_200 As an admin I should not be able to create customer with duplicated email @e2e @customer", async () => {
  const firstName = faker.name.firstName();
  const lastName = faker.name.lastName();
  const note = faker.lorem.sentence();
...
test("TC: WEENSPACE_201 Update customer account info @e2e @customer", async () => {
  const firstName = faker.name.firstName();
  const lastName = faker.name.lastName();
  const email = faker.internet.email();
  const note = faker.lorem.sentence();
...
test("TC: WEENSPACE_202 Deactivate a customer @e2e @customer", async () => {
...
test("TC: WEENSPACE_203 Activate a customer @e2e @customer", async () => {
...
test("TC: WEENSPACE_204 Delete customer from the details page @e2e @customer", async () => {
...
test("TC: WEENSPACE_205 Bulk delete customers @e2e @customer", async () => {
...
test("TC: WEENSPACE_206 As an admin I want to add address to the customer and set it as default shipping @e2e @customer", async () => {
  await customersPage.gotoCustomerDetailsPage(CUSTOMERS.editCustomer.id);
...
test("TC: WEENSPACE_209 As an admin I want to update customer's address and set it as default billing @e2e @customer", async () => {
  await customersPage.gotoCustomerDetailsPage(CUSTOMERS.editCustomer.id);
  await addressesListPage.clickManageAddresses();
...
test("TC: WEENSPACE_210 Delete customer's address @e2e @customer", async () => {
  await customersPage.gotoCustomerDetailsPage(CUSTOMERS.editCustomer.id);
  await addressesListPage.clickManageAddresses();
...
test("TC: WEENSPACE_207 Issue a new gift card for the customer @e2e @customer", async () => {
