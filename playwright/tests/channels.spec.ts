...
test("TC: WEENSPACE_97 Create basic channel @e2e @channels", async () => {
...
test("TC: WEENSPACE_208 Create channel with all settings @e2e @channels", async () => {
  const slugName = new Date().toISOString();
...
test("TC: WEENSPACE_98 Edit channel - transaction flow, allow unpaid, authorize, prio high stock @e2e @channels", async () => {
...
test("TC: WEENSPACE_99 Delete channel @e2e @channels", async () => {
