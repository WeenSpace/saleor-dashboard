...
test("TC: WEENSPACE_105 Issue gift card @e2e @gift", async () => {
...
test("TC: WEENSPACE_106 Issue gift card with specific customer and expiry date @e2e @gift", async () => {
  await giftCardsPage.clickIssueCardButton();
...
test("TC: WEENSPACE_107 Resend code @e2e @gift", async () => {
...
test("TC: WEENSPACE_108 Deactivate gift card @e2e @gift", async () => {
...
test("TC: WEENSPACE_109 Activate gift card @e2e @gift", async () => {
...
test("TC: WEENSPACE_110 Edit gift card @e2e @gift", async () => {
...
test("TC: WEENSPACE_111 Bulk delete gift cards @e2e @gift", async () => {
...
test("TC: WEENSPACE_181 Set gift card balance @e2e @gift", async () => {
  await giftCardsPage.gotoExistingGiftCardView(GIFT_CARDS.giftCardToBeEdited.id);
  await giftCardsPage.clickSetBalance();
  await giftCardsPage.setGiftCardsBalanceDialog.setBalance("34");
  await giftCardsPage.expectSuccessBanner();
});
test("TC: WEENSPACE_182 Export gift card codes in XLSX file @e2e @gift", async () => {
...
test("TC: WEENSPACE_183 Export gift card codes in CSV file @e2e @gift", async () => {
  await giftCardsPage.clickShowMoreMenu();
  await giftCardsPage.clickExportGiftCards();
